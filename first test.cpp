...
std::vector<bool> candidates; // will have the size of requiredBits * dimension, therefore being able to contain all required numbers for a single function evaluation
...
void convertBase(std::vector<bool> candidates)
{
...
void simulatedAnnealing(functions currentFunction)
{
	...
	for (int i = 0; i < dimension * requiredBits; i++)
		candidates.push_back(randomNumber() % 2);
	convertBase(candidates);
